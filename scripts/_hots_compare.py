...
def test_codes(codes):
    ...
    for code in codes:
        try:
            ...
            df = df.rename(columns={c: col_map.get(c.lower().strip(),c) for c in df.columns if c.lower().strip() in col_map})
        ...
        # 尾盘战法
        Strat = type("T", (EOD_Strategy,), {"stock_code": code})
        r = Backtest(df, Strat, cash=100000, commission=0.0003, finalize_trades=True).run()
        if r["# Trades"] > 0:
            results.append({"code":code,"trades":int(r["# Trades"]),"ret":round(r["Return [%]"],2),"sharpe":r.get("Sharpe Ratio",0),"wr":r.get("Win Rate [%]",0)})
    return results

class EOD_Strategy(Strategy):
    def init(self):
...
all_hot = sum(hot_codes.values(), [])
for label, codes in [("热点板块(14只)", all_hot), ("传统中小板(10只)", standard)]:
    r = test_codes(codes)
    rets = [x["ret"] for x in r]
    avg = np.mean(rets) if rets else 0
    pos = sum(1 for x in rets if x>0)/len(rets)*100 if rets else 0
    print(f"\n{label}: 有交易{len(r)}/{len(codes)} 均收益{avg:+.2f}% 正收益比{pos:.0f}%")
    for x in sorted(r, key=lambda x:-x["ret"])[:5]:
        print(f"  {x['code']}: {x['trades']}笔 {x['ret']:+.2f}%")
...
for sector, codes in hot_codes.items():
    ...
    avg = np.mean(rets); pos = sum(1 for x in rets if x>0)/len(rets)*100
    print(f"  {sector}: {len(r)}/{len(codes)} 均收益{avg:+.2f}% 正收益比{pos:.0f}%")
